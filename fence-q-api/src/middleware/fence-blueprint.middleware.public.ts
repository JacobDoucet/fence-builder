...
import { getFenceBlueprintsForUser } from '../queries/fence-blueprint.queries';
import { getFenceOrdersForUser } from '../queries/fence-order.queries';

export function fenceBlueprintGetMiddlewarePublic(repo: () => Repository<FenceBlueprint>) {
        ...
        decodeClientToken$(token)
            .pipe(
                mergeMap((user: ApiUser) => handleRequest(
                    getFenceBlueprintsForUser(user), req, res, next)
                ),
...
export function fenceBlueprintPostMiddlewarePublic(repo: () => Repository<FenceBlueprint>) {
    return (req: Request, res: Response, next: NextFunction) => {
        const token = req.query.token|| req.cookies.token;
        ...
        decodeClientToken$(token)
            .pipe(
                ...
                mergeMap((fenceOrders: FenceOrder[]) => {
                    const fenceOrder = fenceOrders.find(order => order.id === req.body.fenceOrderId);
                    if (!fenceOrder) { return throwError('Invalid fenceOrderId'); }
                    req.body.fenceOrder = fenceOrder;
